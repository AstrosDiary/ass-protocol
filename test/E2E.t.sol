...
contract E2ETest is Test {
    ...
    function setUp() public {
        ...
        // --- production deploy sequence, exactly as Item 4 will script it ---
        ...
        vault = AssVault(payable(factory.newVault(predictedToken, address(0), deployer, "")));
        ...
        exec = new AssSwapExecutor(address(wbnb), deployer);
        ...
        exec.setRouter(address(nativeRfq), true);
        ...
        views = new AssViews(address(vault), payable(address(engine)), address(dist), address(tracker));
    ...
    function _cd(MockRouter, address tokenOut, uint256 amountIn) internal view returns (bytes memory) {
    ...
    function test_E2E_TaxToHolderStocks_FullPipeline() public {
        // 1. Flap dispatches tax revenue to the vault
        ...
        assertEq(engine.cumulativeBnbProcessed(), 20 ether);
    ...
    function test_E2E_OneAssetDown_CarryForward_ThenCatchUp() public {
        ...
        nativeRfq.setMode(MockRouter.Mode.Revert); // RFQ dead this cycle
    ...
    function test_E2E_ViewsReflectRealState() public {
        ...
        assertEq(s.vaultTotalReceivedBnb, 20 ether);
        assertEq(s.vaultTotalReleasedBnb, 20 ether);
        assertEq(s.cumulativeBnbProcessed, 20 ether);
        ...
        for (uint256 i; i < 4; ++i) {
            ...
            assertEq(cards[i].cumulativeSpentWbnb, 5 ether);
